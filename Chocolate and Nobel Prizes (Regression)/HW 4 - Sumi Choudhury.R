...
arrows(4.5,33, 6.25, 31.857, length = 0.1, col = "red") 
text(4.3,34.5, labels = "Sweden", col = "red")
...
text(2,30, labels = paste("Correlation =", round(cor(x$Chocolate_Consumption, x$Nobel_Laureates, use = "complete.obs"),
                                                 digits = 5)))
abline(h=0, lty = 3, col ="forestgreen", lwd = 2) # add a line to the zero point on y-axis
...
# use linear regression summary for hypothesis test 
summary(lm.x)
...
plot(x$Chocolate_Consumption, x$Nobel_Laureates, las = T, 
     ...
     xlab = "Chocolate Consumption(kg/yr/capita)", ylab = "Nobel Laureates per 10 Million Population", pch =20,
     xlim = c(0,15), ylim = c(0,35))
abline(coef(lm.x), col = "red", lwd = 2) # add a regression line
